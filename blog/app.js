...
function render(){
  ...
  $('#posts').innerHTML=visible.map((p,i)=>{const cover=(p.imageDataUrls||[])[0]||p.imageDataUrl||'';return `<article class="post-card ${i===0&&!activeTag&&!searchTerm?'featured':''}">${cover?`<a href="post.html?id=${encodeURIComponent(p.id)}"><img src="${esc(cover)}" alt="${esc(p.title)}"></a>`:''}<div class="post-copy"><div class="meta">${dateText(p.eventDate||p.publishedAt)} · ${esc(p.author)}</div><h3><a href="post.html?id=${encodeURIComponent(p.id)}">${esc(p.title)}</a></h3><p>${esc(p.summary)}</p><div class="post-tags">${(p.tags||[]).map(t=>`<a href="${tagUrl(t)}">#${esc(t)}</a>`).join('')}</div><a class="read-more" href="post.html?id=${encodeURIComponent(p.id)}">Leer la entrada →</a></div></article>`}).join('');
}
function renderWidgets(){const counts=new Map();posts.forEach(p=>(p.tags||[]).forEach(t=>counts.set(t,(counts.get(t)||0)+1)));$('#tags').innerHTML=[...counts.entries()].sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0])).map(([t,n])=>`<a href="${tagUrl(t)}" class="${t.toLocaleLowerCase('es')===activeTag.toLocaleLowerCase('es')?'active':''}">${esc(t)} <span>${n}</span></a>`).join('')||'<span class="muted">Aún no hay etiquetas.</span>';const months=new Map();posts.forEach(p=>{const d=toDate(p.eventDate||p.publishedAt),k=new Intl.DateTimeFormat('es-ES',{month:'long',year:'numeric'}).format(d);months.set(k,(months.get(k)||0)+1)});$('#archive').innerHTML=[...months].map(([m,n])=>`<div><span>${esc(m)}</span><strong>${n}</strong></div>`).join('')}
